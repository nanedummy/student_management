...
export default function TimetableBulkUploadModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await api.post('/timetable/bulk-upload/', formData, {
  ...
  const sampleUrl = `${api.defaults.baseURL.replace(/\/api\/?$/, '')}/api/timetable/bulk-upload/sample/`
      ...
      <div className="card fade-in" style={{ width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div className="card-body">
          ...
          <div style={{ marginBottom: 20 }}>
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>
              Upload an Excel (.xlsx) file to add multiple timetable entries at once. 
              The first two rows (headings and example) will be ignored.
            </p>
            <a href={sampleUrl} download="timetable_sample.xlsx" className="btn btn-outline btn-sm" style={{ marginBottom: 12 }}>
        ...
        <div>
          <div className={`alert ${result.created_count > 0 ? 'alert-success' : 'alert-warning'}`}>
            ...
            <p>Successfully created {result.created_count} timetable entries.</p>
          </div>

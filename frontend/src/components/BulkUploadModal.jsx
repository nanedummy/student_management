...
export default function BulkUploadModal({ onClose, onSuccess }) {
  ...
  const sampleUrl = `${api.defaults.baseURL.replace(/\/api\/?$/, '')}/api/students/bulk-upload/sample/`
      ...
      <div className="card fade-in" style={{ width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div className="card-body">
          ...
          <div style={{ marginBottom: 20 }}>
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>
              Upload an Excel (.xlsx) file to add multiple students at once. 
              The first two rows (headings and example) will be ignored.
            </p>
            <a href={sampleUrl} download="sample.xlsx" className="btn btn-outline btn-sm">
        ...
        <div>
          <div className={`alert ${result.created_count > 0 ? 'alert-success' : 'alert-warning'}`}>
            ...
            <p>Successfully created {result.created_count} students.</p>
          </div>
